...
GLfloat xAngle = 90, yAngle, zAngle;
GLboolean bAmbient;
GLboolean bAttach;
...
void DoKeyboard(unsigned char key, int x, int y) {
	switch (key) {
	case 'a': xAngle += 2; break;
	case 'd': xAngle -= 2; break;
	case 'w': yAngle += 2; break;
	case 's': yAngle -= 2; break;
...
void DoMenu(int value) {
	switch (value) {
	case 1: bAmbient = GL_TRUE; break;
	case 2: bAmbient = GL_FALSE; break;
	case 3: bAttach = GL_TRUE; break;
	case 4: bAttach = GL_FALSE; break;
	}
	glutPostRedisplay();
}
